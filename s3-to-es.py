...
globalVars['awsRegion']             = "ap-south-1"
...
def indexDocElement(es_Url, docData):
    try:
        #headers = { "Content-Type": "application/json" }
        headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
        resp = requests.post(es_Url, auth=HTTPBasicAuth('elastic', 'changeme'), headers=headers, json=docData)
        logger.info(resp)
...
def lambda_handler(event, context):
    ...
    # indexName = globalVars['esIndexPrefix'] + str( datetime.date.today().minute ) + '-' + str( datetime.date.today().second )
    indexName = globalVars['esIndexPrefix'] + (event['Records'][0]['s3']['object']['key'])
    ...
    for line in lines:
        ...
        indexDocElement(es_Url, docData)
